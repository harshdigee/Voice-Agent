 ...
 * Seeds DigeeSell KB into Supabase with OpenAI embeddings for semantic search.
 * Usage: node scripts/seedKnowledgeBase.js
 *
 * Prerequisites:
 *  1. Run supabase_setup.sql in your Supabase SQL editor first
 *  2. .env must have SUPABASE_URL, SUPABASE_SERVICE_KEY, OPENAI_API_KEY
 */
...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_KEY   // uses SERVICE key (rules/ version had wrong var name)
);
...
async function seedChunk(chunk, index) {
  process.stdout.write(`[${index + 1}/${KBChunks.length}] Seeding: ${chunk.title}... `);
  try {
    ...
    if (error) { console.log(`FAILED: ${error.message}`); return false; }
    console.log("OK");
    return true;
  } catch (err) {
...
async function main() {
  ...
  // Verify connection
  const { error: connErr } = await supabase.from("documents").select("id").limit(1);
  if (connErr) {
    console.error("Supabase connection failed:", connErr.message);
    console.error("Did you run rules/supabase_setup.sql in the Supabase SQL editor?");
    process.exit(1);
  }
  ...
  for (let i = 0; i < KBChunks.length; i++) {
    ...
    if (i < KBChunks.length - 1) await new Promise((r) => setTimeout(r, 300));
  }

  console.log("\n═══════════════════════════════════════════");
  console.log(`  Done: ${ok} seeded, ${fail} failed`);
  if (fail === 0) console.log("  KB ready for semantic search!");
  console.log("═══════════════════════════════════════════\n");
